...
function selectPilot() {
  ...
  fetch(`${ROOT}/ApiManager/pilot/selectPilot/${id_pilot}`)
    ...
    .then((data) => {
      ...
      let select = document.getElementById("comboboxCentre");
      console.log(data[0].id_centre);
      select.value = parseInt(data[0].id_centre);
...
function ajoutplus() {
  ...
  if (numberOfSelects < 5) {
    ...
    const selectHTML =
      "<select name='localite[]' class='comboboxPromo' required>" +
